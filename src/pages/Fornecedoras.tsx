      ...
      <div className="filter-bar flex flex-wrap gap-4 items-end">
        <div>
          ...
          <Select value={tipoFilter} onValueChange={setTipoFilter}>
            <SelectTrigger className="w-28 rounded-full bg-muted/50 border-0 mt-1 font-mono text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="flex-1 min-w-[150px]">
          ...
          <div className="relative mt-1">
            ...
            <Input className="pl-9 rounded-full bg-muted/50 border-0 font-mono text-xs" placeholder="Nome..." value={busca} onChange={e => setBusca(e.target.value)} />
          </div>
        </div>
        <Button onClick={openNew} className="rounded-full bg-accent text-accent-foreground hover:bg-accent/90 font-mono text-xs">
          <Plus className="h-4 w-4 mr-1" /> NOVA FORNECEDORA
        </Button>
      </div>

      <p className="text-xs text-muted-foreground font-mono">{filtered.length} fornecedoras</p>
            ...
            >
              <div className="flex items-start gap-4 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!f.ativa && <Badge variant="outline" className="text-[10px] mt-1 rounded-full font-mono">Inativa</Badge>}
                </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-md overflow-hidden rounded-2xl">
          <DialogHeader className="bg-primary -mx-6 -mt-6 px-6 pt-6 pb-4 mb-2">
            <DialogTitle className="font-display text-xl text-primary-foreground tracking-wide">{editing ? 'EDITAR' : 'NOVA'} FORNECEDORA</DialogTitle>
          </DialogHeader>
          <div className="space-y-5">
            <div><Label className="label-upper">Nome</Label><Input value={nome} onChange={e => setNome(e.target.value)} className="mt-1.5 rounded-xl font-mono text-xs" /></div>
            <div><Label className="label-upper">Contato (telefone)</Label><Input value={contato} onChange={e => setContato(e.target.value)} className="mt-1.5 rounded-xl font-mono text-xs" /></div>
            <div><Label className="label-upper">Chave Pix</Label><Input value={chavePix} onChange={e => setChavePix(e.target.value)} className="mt-1.5 rounded-xl font-mono text-xs" /></div>
            <div><Label className="label-upper">Observações</Label><Textarea value={observacoes} onChange={e => setObservacoes(e.target.value)} className="mt-1.5 rounded-xl font-mono text-xs" /></div>
            <div className="flex items-center gap-6 pt-2">
              <div className="flex items-center gap-2">
                <Switch checked={ativa} onCheckedChange={setAtiva} /><Label className="font-mono text-xs">Ativa</Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch checked={ehSocia} onCheckedChange={setEhSocia} /><Label className="font-mono text-xs">Sócia</Label>
              </div>
            </div>
          </div>
          <DialogFooter className="mt-5">
            <Button variant="outline" onClick={() => setShowForm(false)} className="rounded-full font-mono text-xs">Cancelar</Button>
            <Button onClick={handleSave} disabled={!nome} className="rounded-full bg-accent text-accent-foreground hover:bg-accent/90 font-mono text-xs">Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {detailForn && (
            <>
              <SheetHeader className="pb-5 border-b">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {detailForn.contato && <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-1 font-mono"><Phone className="h-3 w-3" /> {detailForn.contato}</p>}
                  </div>
                </div>
              </SheetHeader>
              ...
              <div className="mt-6 space-y-6">
                {detailForn.chavePix && (
                  <p className="text-xs flex items-center gap-2 text-muted-foreground font-mono"><Key className="h-4 w-4" /> {detailForn.chavePix}</p>
                )}
                {detailForn.observacoes && (
                  <p className="text-xs text-muted-foreground bg-muted/40 p-4 rounded-2xl font-mono">{detailForn.observacoes}</p>
                )}
                ...
                {detailVendas.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {detailVendas.map(v => (
                        <div key={v.id} className="flex justify-between items-center p-4 rounded-2xl bg-card border text-xs hover:bg-muted/20 transition-all duration-200">
                          <div>
                            <span className="font-mono text-muted-foreground">#{v.skuPeca}</span>{' '}
                            <span className="font-medium font-mono">{v.descricaoPeca}</span>
                            <p className="text-[10px] text-muted-foreground mt-0.5 font-mono">{v.dataVenda} • {v.compradora || 'Sem compradora'}</p>
                          </div>
                ...
                {detailPecas.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto">
                      {detailPecas.map(p => (
                        <div key={p.sku} className="flex justify-between items-center p-3 rounded-xl bg-card border text-xs hover:bg-muted/20 transition-all duration-200">
                          <span className="font-mono"><span className="text-muted-foreground">#{p.sku}</span> {p.descricao}</span>
                          <div className="flex items-center gap-2">
                            <span className="font-mono-price text-primary">{fmt(p.preco)}</span>
