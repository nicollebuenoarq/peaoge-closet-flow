...
export const supabaseStore = {
    ...
    async getFornecedoras(): Promise<Fornecedora[]> {
          const { data, error } = await supabase.from('fornecedoras').select('*').order('nome');
          if (error) throw error;
          return (data ?? []).map(dbToFornecedora);
    },

    async setFornecedoras(list: Fornecedora[]): Promise<void> {
          const { error } = await supabase.from('fornecedoras').upsert(list.map(fornecedoraToDb));
          if (error) throw error;
    },

    async upsertFornecedora(f: Fornecedora): Promise<void> {
          const { error } = await supabase.from('fornecedoras').upsert(fornecedoraToDb(f));
          if (error) throw error;
    },

    async deleteFornecedora(id: string): Promise<void> {
          const { error } = await supabase.from('fornecedoras').delete().eq('id', id);
          if (error) throw error;
    },

    // PECAS
    async getPecas(): Promise<Peca[]> {
          const { data, error } = await supabase.from('pecas').select('*').order('sku');
          if (error) throw error;
          return (data ?? []).map(dbToPeca);
    },

    async setPecas(list: Peca[]): Promise<void> {
          const { error } = await supabase.from('pecas').upsert(list.map(pecaToDb));
          if (error) throw error;
    },

    async upsertPeca(p: Peca): Promise<void> {
          const { error } = await supabase.from('pecas').upsert(pecaToDb(p));
          if (error) throw error;
    },

    async deletePeca(sku: number): Promise<void> {
          const { error } = await supabase.from('pecas').delete().eq('sku', sku);
          if (error) throw error;
    },
    ...
    async getVendas(): Promise<Venda[]> {
          ...
          if (error) throw error;
          return (data ?? []).map(dbToVenda);
    },

    async setVendas(list: Venda[]): Promise<void> {
          const { error } = await supabase.from('vendas').upsert(list.map(vendaToDb));
          if (error) throw error;
    },

    async upsertVenda(v: Venda): Promise<void> {
          const { error } = await supabase.from('vendas').upsert(vendaToDb(v));
          if (error) throw error;
    },
    ...
    async getLembretes(): Promise<Lembrete[]> {
          const { data, error } = await supabase.from('lembretes').select('*').order('criado_em');
          if (error) throw error;
          return (data ?? []).map(dbToLembrete);
    },

    async setLembretes(list: Lembrete[]): Promise<void> {
          const { error } = await supabase.from('lembretes').upsert(list.map(lembreteToDb));
          if (error) throw error;
    },

    async upsertLembrete(l: Lembrete): Promise<void> {
          const { error } = await supabase.from('lembretes').upsert(lembreteToDb(l));
          if (error) throw error;
    },
    ...
    async getDropPlans(): Promise<DropPlan[]> {
          const { data, error } = await supabase.from('drop_plans').select('*').order('drop');
          if (error) throw error;
          return (data ?? []).map(dbToDropPlan);
    },

    async setDropPlans(list: DropPlan[]): Promise<void> {
          const { error } = await supabase.from('drop_plans').upsert(list.map(dropPlanToDb));
          if (error) throw error;
    },

    async upsertDropPlan(d: DropPlan): Promise<void> {
          const { error } = await supabase.from('drop_plans').upsert(dropPlanToDb(d));
          if (error) throw error;
    },

    async deleteDropPlan(drop: number): Promise<void> {
          const { error } = await supabase.from('drop_plans').delete().eq('drop', drop);
          if (error) throw error;
    },
    ...
    async setConfig(c: AppConfig): Promise<void> {
          const { error } = await supabase.from('config').upsert(configToDb(c));
          if (error) throw error;
    },

    // NEXT SKU
    async getNextSku(): Promise<number> {
          const { data, error } = await supabase.from('config').select('next_sku').eq('id', 1).single();
          if (error || !data) return 1;
          return data.next_sku ?? 1;
    },

    async setNextSku(n: number): Promise<void> {
          const { error } = await supabase
            .from('config')
            .upsert({ id: 1, next_sku: n, updated_at: new Date().toISOString() });
          if (error) throw error;
    },
